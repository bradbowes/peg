...
struct
   datatype 'a map =
        LEAF
      | NODE of 'a map * (string * 'a) * 'a map * int
   ...
   fun insert (tbl, key, value) =
      let
         ...
         fun rotate_left (NODE (l, v, NODE (rl, rv, rr, _), _)) =
           ...
           | rotate_left _ = raise (Fail "rotate_left")

         fun rotate_right (NODE (NODE (ll, lv, lr, _), v, r, _)) =
               new_node (ll, lv, new_node (lr, v, r))
           | rotate_right _ = raise (Fail "rotate_right")

         fun get_key (NODE (_, (k, _), _, _)) = k
           | get_key _ = raise (Fail "get_key")
            ...
            NODE (l, v, r, h) =>
               if balance > 1 then
                  let
                     val k = get_key l
                  in
                     if key < k then rotate_right t
                     else if key > k then rotate_right (NODE (rotate_left l, v, r, h))
                     else t
                  end
                  ...
                  let
                     val k = get_key r
                  in
                     if key > k then rotate_left t
                     else if key < k then rotate_left (NODE (l, v, rotate_right r, h))
                     else t
                  end
               else
                  t
